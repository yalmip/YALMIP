function output = callknitro_milp(interfacedata)

model = yalmip2knitromiqp(interfacedata);

showprogress('Calling KNITRO',interfacedata.options.showprogress);
solvertime = tic;
[x,fval,exitflag,output,lambda] = knitro_milp(model.f,model.xType, model.A,model.b,model.Aeq,model.beq,model.lb,model.ub,model.x0,model.ops);
