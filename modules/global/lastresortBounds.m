...
if p.options.bmibnb.sdpbounder && isempty(p.evalMap) && all(p.variabletype <=2)
    % Only do this very badly bounded, or user really wants it
    if (all(isinf(p.ub)) || all(isinf(p.lb))) || p.options.bmibnb.sdpbounder>=1
        % QP with all unbounded variables. This never ends well.
        % Try to solve a semidefinite relaxation to obtain an upper bound on
        % x'*x.
        % Currently only implemented when there are linear constraints, can be
        % generalized in absurdum. x'*x <= gamma when f+c'*x+x'*Q*x <= U, Ax == b
        ...
        if size(A,1) > 0
            ...
            H3 = [b -A]'*[b -A];
            if ~isinf(upper)
                sol = optimize([[H1 - tau1*H2 - tau2*H3]>=0, tau1>=0],gamma,sdpsettings('verbose',p.options.verbose));
                if sol.problem == 0
                ...
                % used later
                try
                    p.boundGenerator.optimizer = optimizer([[H1 - tau1*H2 - tau2*H3]>=0, tau1>=0],gamma,sdpsettings('solver','mosek','verbose',p.options.verbose),U,gamma);
