function [Q,c] = compileQuadratic(c,p,onlysquares,canshift)
if nargin < 4
    canshift = ones(length(c),1);
end
...
for i = 1:size(p.bilinears,1)
    ...
    if onlysquares == 3 && quadratic
        if c(p.bilinears(i,1))>0 && canshift(p.bilinears(i,1))
            Q(p.bilinears(i,2),p.bilinears(i,3)) = c(p.bilinears(i,1));            
            c(p.bilinears(i,1)) = 0;
        end
    elseif c(p.bilinears(i,1)) && (~onlysquares || quadratic) && ~(onlysquares == 3) && canshift(p.bilinears(i,1))
