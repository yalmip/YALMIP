...
assert(abs(double(x) - 1.25) <= 1e-5)

sol = solvesdp([uncertain(w),-2<=w<=4,5-x*w>=0],x)
assert(abs(double(x) - -2.5) <= 1e-5)

F = (abs(x) + w <= 1) + (norm(w,1) <= 0.3);
sol = solverobust(F,-x,[],w);
assert(abs(double(x) - 0.7) <= 1e-5)
assert(sol.problem ==  0)
...
assert(abs(double(sum(x)) - 0.7) <= 1e-5)
assert(sol.problem == 0)
...
sol = solverobust(F,-sum(x),[],w);
assert(abs(double(sum(x)) - 0.57574) <= 1e-5)
assert(sol.problem == 0)
...
sol = solverobust(F,-sum(x),[],w);
assert(abs(double(sum(x)) - 0.8) <= 1e-5)
assert(sol.problem == 0)
