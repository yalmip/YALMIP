function tests = test_fmincon_as_sdpsolver
...
testCase.assertTrue(isequal(sol.problem,0));
testCase.assertTrue(abs(value(trace(Y)-10.26039))<1e-3);
...
sol = optimize(F,-trace(Y),sdpsettings('solver','fmincon'));
testCase.assertTrue(isequal(sol.problem,0));
testCase.assertTrue(abs(value(trace(Y)-3.5497))<1e-1);
...
testCase.assertTrue(isequal(sol.problem,0));
testCase.assertTrue(abs(value(trace(Y)-10.26039))<1e-1);
