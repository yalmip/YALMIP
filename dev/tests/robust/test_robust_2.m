...
sol = solverobust(F,-trace(Y),sdpsettings('verbose',0),alpha);

K = value(L)*inv(value(Y));

testCase.assertTrue(sol.problem == 0)
testCase.assertTrue(norm(K -   [-1.3674   -2.9158   -2.6670]) <= 1e-2)
...
Y0 = value(Y0);
Y1 = value(Y1);
testCase.assertTrue(abs(max([trace(Y0-0.1*trace(Y1)) trace(Y0+0.1*trace(Y1))])-2.3319) <= 1e-3)
