...
U = reshape(1:6,2,3);
Z = P{U};
testCase.assertTrue(norm(U-Z)<= 1e-10);
...
U = reshape(1:6,2,3);
Z = P{U};
testCase.assertTrue(norm(U-Z(:,:,1))<1e-10);
testCase.assertTrue(norm(2*(U)-imag(Z(:,:,2)))<1e-10);

U = reshape(1:6,2,3) + sqrt(-1)*reshape(7:12,2,3);
Z = P{U};
testCase.assertTrue(norm(U-Z(:,:,1)) <= 1e-5);
testCase.assertTrue(norm(2*real(U)-imag(Z(:,:,2))) <= 1e-5);
testCase.assertTrue(norm(2*imag(U)-real(Z(:,:,2))) <= 1e-5);
...
U = reshape(1:6,2,3) + sqrt(-1)*reshape(7:12,2,3);
Z = P{U};
testCase.assertTrue(norm(Z(:,:,1)-reshape(7:12,2,3))<1e-7);
...
U2 = -reshape(1:6,2,3);
Z = P{{U1,U2}};
testCase.assertTrue(norm(Z-reshape(7:12,2,3))<1e-8);
