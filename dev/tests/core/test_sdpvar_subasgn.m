...
testCase.assertTrue(t2)
yalmip('clear')
x = sdpvar(1,5);
xx = x;
x(2:3) = [-x(1) x(1)];
t3 = isequal(x-[xx(1) -xx(1) xx(1) xx(4:5)],zeros(1,5));
testCase.assertTrue(t3)
...
testCase.assertTrue(t4)
yalmip('clear')
x = sdpvar(1,5);
xx = x;
x(1:3) = [-x(1)];
t5 = isequal(x-[-xx(1) -xx(1) -xx(1) xx(4:5)],zeros(1,5));
testCase.assertTrue(t5)
yalmip('clear')
x = sdpvar(1,5);
y = sdpvar(1,5);
xx = x;
...
testCase.assertTrue(t6)
x = sdpvar(1,5);
y = sdpvar(1,5);
xx = x;
x(1:4) = y(1:4);
t7 = isequal(x-[y(1:4) xx(5)],zeros(1,5));
testCase.assertTrue(t7)
...
testCase.assertTrue(t13)
% Bug 1006
p = sdpvar(3,1);
p([1 1])=[4 5];
t14 = isa(p(1),'double') & isequal(p(1),5);
testCase.assertTrue(t14)
% Bug #190
x = sdpvar(2);
y = x;
y(:,2) = [];
t15 = isequal(y-x(:,1),[0;0]);
testCase.assertTrue(t15)